...
def test_the_package_imports() -> None:
    import comeni_ai  # noqa: F401


def test_the_package_ships_a_py_typed_marker() -> None:
    assert (ROOT / "packages" / "comeni-ai" / "src" / "comeni_ai" / "py.typed").exists()
...
def test_the_root_project_depends_on_it() -> None:
    ...
    assert '"comeni-ai"' in manifest
    assert "comeni-ai = { workspace = true }" in manifest


def test_it_is_findable_as_an_installed_distribution() -> None:
    assert importlib.util.find_spec("comeni_ai") is not None
