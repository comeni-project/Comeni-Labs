...
| 17m | [`2026-08-19-plan-3d-the-forge-redesigned.md`](plans/2026-08-19-plan-3d-the-forge-redesigned.md) | **Plan 3D — WRITTEN 2026-08-19, not started.** Six phases, 56 steps, against `specs/2026-08-19-the-forge-redesigned.md`. **Phase 1 is the whole point and it is measured before it is written**: over the 30 ports of the 12 landed contracts, alphabetical ordering — what ships today — puts the right type first in **1 of 30 holes (3%)**; ranking by the port name, the tool's own name and the namespace of what the module consumes puts it first in **25 of 30 (83%)**, which clears the spec's 80% bar with pure arithmetic over declared data and no model. Phase 1 then writes the producer `OpenQuestion.suggested` never had. Phases 4 and 6 are **deliberately thinner** and say so: they are written against a `Board` type phase 3 creates, and this file's own row 17b records that writing later phases against types that do not exist is what killed Plan 2 and the original Plan 3. Phase 6 is the landing page, **last** |
