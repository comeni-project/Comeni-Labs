...
| 17n | [`2026-08-19-plan-3c-the-builder.md`](plans/2026-08-19-plan-3c-the-builder.md) + [`../docs/design/dashboard.md`](../docs/design/dashboard.md) | **Plan 3C — WRITTEN 2026-08-19, not started.** Nine phases, 52 steps, **three operator checkpoints** — after the shell, after the graph, and at the end. The checkpoints exist because 3D's interface was rejected three times, and the cause was that nobody had designed those screens and they got designed while being built. **3C does not have that problem**: `dashboard.md` §4–6 gives resize ranges in pixels, zoom clamps, corner radii and which settings groups open by default, and `dashboard.html` is 922 lines of working pan, zoom and orthogonal routing. So the instruction is *do not invent screens*. Phases 0 and 1 are the two prerequisites every earlier plan named: the orchestration seam — a **lift, not a rewrite**, since `resolve_verbs`' own comment says the core needs no disk — and **DAG layout in Python rather than the browser**, so the thing a person screenshots gets the same determinism `.nf` has. **Two parts of the design are deliberately out of scope and said so up front**: the *Ask Mendel* tab is door 1, and drag-to-connect is `dashboard.md` §9's own gap |
