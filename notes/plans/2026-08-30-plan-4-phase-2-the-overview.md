...
- [ ] Add `list_drafts()` to `packages/mendel-api/src/mendel_api/services/drafts.py`, ordered by
      `updated_at` descending, paginated.
- [ ] Each row carries: `id`, `name`, `who`, `updated_at`, whether it has been **kept**, the
      digest of the kept `pipeline.yml`, the step count, and the **provenance counts**
      (settled / measured / open).
- [ ] **Provenance comes from the stored artifact.** `keep()` already writes a `pipeline.yml`
      ...
      truth.
- [ ] Add the route to `routes/build.py` following that file's shape — a declared out model, an
      `operation_id`, a one-line `summary`. Remember the router's prefix: the path is
      `/api/pipeline/drafts`.
- [ ] Name the open values. A pipeline with two tier-4 settings must return *which two*, by name,
...
- [ ] Add `test_the_listing_never_resolves` — patch the resolver's entry point to raise, and
...
- [ ] Add `test_an_unkept_draft_has_no_provenance` — absent, never three zeroes. Watch it fail.
- [ ] Measure it. The budget is the one the 2026-08-19 audit set: nothing on this page may cost
...
- [ ] In `packages/wiener-api/src/wiener_api/services/artifacts.py`, read the `pipeline.yml` out
      of the uploaded bundle and record its digest on the `RunArtifact` row at upload time.
- [ ] Use the digest **Mendel already computes and writes into the artifact**, not a fresh hash of
...
- [ ] If the bundle carries no readable `pipeline.yml`, leave it `None`. A wrong key is worse than
      an absent one, and the column is already nullable.
- [ ] Backfill is **not** in scope. Existing rows keep `None` and the page shows those runs under
      *every run* without a pipeline. Say so in the model docstring.
- [ ] Add `test_an_uploaded_artifact_records_which_pipeline_it_is`. **Watch it fail** — it fails
...
- [ ] Add `durations_by_pipeline(session, lab_id, *, days)` to
...
- [ ] **Fold in Python**, following `board_summary()`'s stated argument — a median has no portable
      SQL spelling across SQLite and Postgres, and the window is hundreds of runs. Do not reopen
      that decision; cite it.
- [ ] A group with fewer than a floor of runs returns **no median**. *Usually 38m* over two runs
      is not a usual, and `rn-board` is explicit that this number earns its place by being a
      judgement rather than trivia.
- [ ] Expose it on the existing `GET /api/runs/summary` rather than a new route — it is the same
      question the tiles already ask, at a different grouping.
- [ ] The **resource sentence** aggregates across runs and is legitimate at lab scale and
...
- [ ] Add `test_a_median_needs_enough_runs_to_be_one`. Watch it fail.
- [ ] `make client`.
...
- [ ] Stop rendering the `forge` calls. **Do not delete `whats_open()`'s forge half** — the forge
      ...
      the date.
- [ ] Fill the `mendel` half. Its docstring currently reads *"Empty today, and that is a section
...
- [ ] `Urgency` keeps its three members and its declared `rank`. An open tier-4 value is
      `WAITING`, not `BLOCKING` — nothing is broken, somebody is held up.
- [ ] Delete `Standing` from the payload. `ov-settled` puts the registry inventory off this page
...
- [ ] Rewrite `test_the_standing_says_what_the_registry_holds` out of existence and
...
- [ ] **Restate** rather than delete. The new page renders **pipelines and runs, never a contract,
...
- [ ] Add a dated paragraph to `docs/design/forge-review.md` §3 recording that the constraint was
...
- [ ] Watch the restated tests fail: render a contract id on the page and confirm each goes red.
...
- [ ] **NOW** — renders only if something is running **or** something is waiting on a person.
      Not a card saying nothing is happening. Not a zero.
- [ ] **Work** — one block with a by-pipeline / by-run toggle, defaulting to **by pipeline**,
...
- [ ] **The same table shape for both**, with columns that differ by object.
...
- [ ] **Do not leak run information onto a pipeline card.** *"last run 2d ago · M. Silva"* on a
      pipeline row was the actual bug behind the two blocks reading as one list rendered twice.
- [ ] The **provenance bar** is one stacked bar **per pipeline** — settled / measured / open —
...
- [ ] **The person slot.** `submitted_by` is hardcoded `"operator"` at submit and `Run.submitted_by`
...
- [ ] **Results ready** — renders only if a run has published outputs, which Phase 1 made
      possible. Link to them.
- [ ] **The resource sentence** — renders only once there are enough runs to aggregate honestly.
- [ ] **The first-run state is its own composition** — one question, one field. Not this page with
      everything hidden, and not onboarding cards. `OverviewFirst.dc.html` is the board.
- [ ] Join pipelines to runs **in the browser**, on the pipeline digest. Write the comment
...
- [ ] Delete `frontend/src/home/Standing.tsx`. Review `frontend/src/home/Artifact.tsx` — if the
...
- [ ] Add `it("is shorter when nothing is happening")` — the `Overview` / `OverviewQuiet`
...
- [ ] Add `it("names the values waiting on a person")` — assert the setting names appear and no
...
- [ ] Write `notes/journal/2026-08-30-the-overview.md`: what shipped, what the design notes got
      wrong (P2-1 and P2-4 especially), what was found by running it rather than by a test, and
      what the next phase inherits.
- [ ] Update `CLAUDE.md`'s journal pointer **in this commit**. It went three entries stale before
      anybody noticed and the file now carries a line saying so.
- [ ] Add the Plan 4 rows to [`../README.md`](../README.md)'s ordered table.
