...
- [ ] Click an **output** → what accepts it. Click an **input** → what produces it. Both filtered
      through the compatibility index, which is the same index that colours a wire mid-drag.
- [ ] Show the **filtered count against the total** — *6 of 1,604*. #77 means the total is
...
- [ ] Search sits **inside** the popover, for when six becomes sixty. It is not how the list comes
      to exist.
- [ ] Choosing one **adds the step and draws the wire** — the whole point. Adding from a port is
      also what fixes placement by construction: the new node has a place to go.
- [ ] Keyboard-first: focus on open, arrows to move, Enter to take, Escape to close.
- [ ] Add `it("offers only what the index says fits")` — watch it fail by dropping the filter.
- [ ] Add `it("never parses a signature in the browser")` — a scan asserting no `split("[")` or
