...
**History is not rewritten.** Roughly 120 further occurrences live in `notes/journal/`,
`notes/audits/` and the 2026-08-05 conformance plan. Those are append-only and were correct on
...
- `mendel_compiler/conformance.py` — `where` replaces `contract_id`; `MD0108`, `MD0200`–`MD0214`;
  ...
  `docs/design/conformance.md`. **`notes/journal/`, `notes/audits/` and the
...
1. **Rebase this branch.** It is based on `e9cab07` and touches three files. `CLAUDE.md` and
   `notes/README.md` will conflict — 1.9 marks roots closed and moves statuses in both — and
