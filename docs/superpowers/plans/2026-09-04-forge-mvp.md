...
- [ ] Implement the records in §3 with `extra="forbid"` and stable serialization.
- [ ] Implement conditional HTTP, pagination, retries with bounded backoff, stable evidence ids,
  and digest helpers in the base.
- [ ] Add one narrow `httpx` dependency to `mendel-forge`; inject the client in tests.
- [ ] Refuse truncated/incomplete catalogues instead of publishing a false total.
- [ ] Preserve the existing `Observation` shape or provide an explicit adapter into it.
- [ ] Test two fake source subclasses with different capabilities.
...
- [ ] Implement §4 exactly, including pagination and per-tool content digests.
- [ ] Parse current nf-core `meta.yml`; keep `ModuleSpec` as the process parser.
- [ ] Normalize Docker Hub metadata and OCI digest/platform data.
- [ ] Join PEGiS Docker Hub entries to `pegi3s/dockerfiles` evidence without scraping HTML.
- [ ] Show unsupported PEGiS entries rather than silently filtering them.
- [ ] Assert source counts are derived from fixture contents, never literals.
- [ ] Assert an unrelated nf-core tree change does not mark a module outdated.
...
- [ ] Store normalized metadata as JSON only where source-specific shape genuinely varies.
- [ ] Give adaptations opaque ids and optimistic `row_version`.
- [ ] Enforce one active adaptation per catalogue item with a partial unique index where supported
  and service-level refusal everywhere.
- [ ] Implement every state transition and its allowed predecessors in one module.
- [ ] Record UTC timestamps for state facts, never derive truth from file mtimes.
- [ ] Keep provider keys and full credentials out of every table.
...
- [ ] Write immutable bundle directories under `workspace/forge/<adaptation-id>/`.
- [ ] Split source, scaffold, candidate, prompt, response, and validation artifacts into named
  subdirectories; validate every joined path.
- [ ] Generate stable semantic hole ids and current pointers.
- [ ] Copy nf-core source unchanged and generate only contract/provenance scaffolding.
- [ ] Replace the one-input/`*.out` PEGiS assumption with explicit marked open sections.
- [ ] Assert two scaffolds from the same source and registry digests are byte-identical.
- [ ] Assert no prompt or scaffold contains an absolute host path.
- [ ] Keep the Forge write-boundary guard: workspace writes are allowed; registry writes remain
...
- [ ] Implement dossier selection and deterministic budget reduction.
- [ ] Implement the exact analysis, implementation, repair, and chat constraints in §5.
- [ ] Validate every evidence id against the dossier.
- [ ] Render proposals into candidate files; never let a response choose a destination path.
- [ ] Run the extended validation ladder and at most two repair passes.
- [ ] Store rendered prompt/response plus their digests for each invocation.
- [ ] Build the offline corpus and `make forge-ai-eval` command.
...
- [ ] `AIWorkerSettings.queue_name = "arq:queue:ai"` and configurable `max_jobs`, default 1.
- [ ] Give enqueue calls explicit job ids based on adaptation/revision/purpose.
- [ ] Claim expected state/version before work.
- [ ] Add maximum job duration and a startup recovery query for abandoned running states.
- [ ] Sanitize provider failures into coded diagnostics while retaining internal detail in logs.
- [ ] Keep source sync off the AI queue.
