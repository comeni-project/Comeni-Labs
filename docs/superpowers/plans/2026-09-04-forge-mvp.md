...
**The three ordinary job bodies are absent, and there is no stub for them.** A function raising
`NotImplementedError` on a worker's function list is worse than an absent one: it is enqueueable,
so the failure arrives at run time on a real adaptation instead of at the call site. Their
`enqueue_*` helpers **are** built and tested — which queue, and what the id is keyed on — because
those are the decisions that are expensive to change once a job has been landing somewhere in
production. `test_no_job_on_a_worker_list_is_an_unimplemented_stub` holds the line.
