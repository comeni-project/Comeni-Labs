...
- [ ] Run a PEGiS adaptation with local Ollama by hand, request one revision, ask one chat
  question, approve, and load the resulting registry. — **OPERATOR**
- [ ] Build a pipeline that resolves through the newly published contract and run the narrowest
  meaningful stub/test gate. — **OPERATOR** (the layer-loads-back half is automated)
- [ ] Compare the running pages beside the rendered artboards. — **OPERATOR**
