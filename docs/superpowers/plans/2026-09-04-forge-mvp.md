...
**`sync_forge_sources` is built; `scaffold_forge_adaptation` and `publish_forge_adaptation` are
not, and there is no stub for either.** A function raising `NotImplementedError` on a worker's
