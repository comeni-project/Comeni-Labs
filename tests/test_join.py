...
def test_two_samples_join_pairwise_and_combine_cross_products(tmp_path, expression, expected):
    ...
    for name in ("s1.a", "s1.b", "s2.a", "s2.b"):
        (tmp_path / f"{name}.tsv").write_text(_declared(tmp_path / f"{name}.tsv", f"{name}\n"))
    (tmp_path / "pairup.nf").write_text(_declared(tmp_path / "pairup.nf", MODULE))
    (tmp_path / "main.nf").write_text(
        _declared(
            tmp_path / "main.nf",
            WORKFLOW.format(expression=expression)))
